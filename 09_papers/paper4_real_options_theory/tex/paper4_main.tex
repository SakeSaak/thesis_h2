...
\author{Sake Saakstra\thanks{Vrije Universiteit Amsterdam. Email: \texttt{s.saakstra@vu.nl}. I am grateful to Siem Jan Koopman and Nadine Ketel for guidance, and to seminar participants at VU Amsterdam for valuable feedback. All errors are mine.}}
...
\paragraph{Note on empirical identification.}
The proposition is a theoretical result under the model's regularity conditions and Assumption~\ref{ass:policy-advantage}. Its direct empirical identification requires exogenous variation in the credibility belief $\pi$ that is independent of payoff-volatility $\sigma$. Three formal identification tests reported in the companion thesis \citep{saakstra2026thesis}, Appendix~A.14 --- a pooled regression with continuous policy-uncertainty and volatility proxies (BBD-EPU and VIX/EWMA), a channel-stratified offtake-decomposition (chemical-feedstock $\mu$-proxy versus power-heat $\sigma$-proxy versus transport $\eta$-proxy), and an event-study with the EU Green Deal, COVID, Ukraine, and IRA shocks --- converge on the finding that no such exogenous variation is identifiable in the observational sample. The proposition therefore functions as a theoretical organising framework and as a generator of testable empirical signatures (notably the structural break in $\bint(t)$ documented in \cite{saakstra2026paper1}) rather than as a directly identified causal channel. The empirical signature is robust; its channel-attribution between $\pi$, $\sigma$, and expectations dynamics is empirically underdetermined. This identification limit constitutes a natural research-design target for follow-up work using natural-experiment regime discontinuities (constitutional carbon-budget amendments, election-driven climate-policy reversals) that would isolate $\pi$ from $\sigma$.
