...
The observation-driven framework of \cite{creal2008general, creal2013generalized} introduced the Generalized Autoregressive Score (GAS) family, in which the time-varying parameter is updated each period by a recursion driven by the score of the predictive log-likelihood. \cite{harvey2013dynamic} provides the textbook treatment of the family and the conditional-score-test interpretation. \cite{blasques2015robust} establish that under a class of regularity conditions the score-driven recursion is the unique observation-driven scheme that is locally optimal in the Kullback-Leibler scoring rule sense; this constitutes the asymptotic-efficiency rationale for choosing score-driven over alternative parameter-update rules. Applications of the GAS framework span volatility modelling, count-data dynamics, and time-varying correlations; the present paper applies the framework to survival modelling, an application area that complements the existing score-driven literature. Particularly close in spirit to the regime-sensitive structural-break dynamics documented in our empirical application is the time-varying-parameter formulation of \citet{blasques2022local}, who develop a score-driven specification capable of accommodating local explosions in the parameter trajectory --- a methodological feature that we draw on to interpret the discontinuous intensification of $\bint(t)$ around the 2020 European Green Deal regime boundary. More broadly, \citet{blasques2024mitigating} situate the use of score-driven and other observation-driven specifications within an explicit programme of \emph{mitigating} estimation risk in observational data --- an epistemic position that aligns directly with the disciplined identification framing adopted throughout this paper and the companion thesis \citep{saakstra2026thesis}.
...
The TVP extension of the Cox proportional-hazards model has been studied in the biostatistics literature primarily through parameter-driven kernel or spline approaches. Observation-driven extensions appear to have received less attention in this setting; adjacent literatures on dynamic frailty models \citep{robins2000marginal} and on score-driven count-data dynamics \citep{creal2013generalized} contain methodological neighbours that we draw on for our specification. The closest precedent is the dynamic-frailty literature \citep{robins2000marginal}, in which the latent frailty term itself follows a Markov process; the GAS formulation we develop is a different generalisation, allowing the regression coefficient itself rather than an unobserved frailty to vary over time according to a score-driven update.
...
The application uses a sample of 714 hydrogen projects spanning announcement-stage status across the 2010--2026 period. The sample is drawn from a methodological-development precursor compilation that combines records from the S\&P Global Commodity Insights hydrogen-project database, the International Energy Agency Hydrogen Production Projects database, and project-disclosure documents from major sponsors. This precursor sample is distinct from the larger S\&P-only sample ($N = 1{,}354$ projects) used in companion empirical analyses archived under DOI 10.5281/zenodo.20359771. The precursor sample is retained for the methodological-development application of this paper because the TVP specifications, prior structure, and posterior-inference machinery were developed and validated on this compilation. Companion empirical analyses on the larger S\&P-only sample re-estimate the substantive cross-jurisdictional findings under matching and difference-in-differences identification strategies, and the within-paper conclusions of the present methodological contribution do not depend on the precursor-versus-S\&P-only distinction. The sample is restricted to projects with the principal technology classification of either Fossil-with-CCS (``Blue''; $n = 158$) or Polymer-Electrolyte-Membrane Electrolysis (``Green''; $n = 556$), with announcement years between 2010 and 2026.
