...
The TVP extension of the Cox proportional-hazards model has been studied in the biostatistics literature primarily through parameter-driven kernel or spline approaches. The observation-driven extension, to our knowledge, has seen only limited application in this setting; we are not aware of an explicit prior formulation of score-driven TVP for discrete-time hazard modelling with sparse events, although adjacent literatures on dynamic frailty models \citep{robins2000marginal} and on score-driven count-data dynamics \citep{creal2013generalized} contain methodological neighbours. The closest precedent is the dynamic-frailty literature \citep{robins2000marginal}, in which the latent frailty term itself follows a Markov process; the GAS formulation we develop is a different generalisation, allowing the regression coefficient itself rather than an unobserved frailty to vary over time according to a score-driven update.
