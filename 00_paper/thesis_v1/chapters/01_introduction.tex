...
\paragraph{Theoretical.} It develops a Dixit-Pindyck (1994) real-options framework that characterises when each carrot-mechanism type dominates as a function of project-level revenue uncertainty and value-to-investment ratio (Chapter~\ref{ch:theory}). The framework yields testable cross-sector predictions about which mechanism types should be more effective in which sectors --- predictions that are subsequently validated empirically.
...
\paragraph{L1 --- Descriptive.} Patterns documented without counterfactual or interventional comparison (Kaplan-Meier survival curves, multistate transition counts, cumulative cancellation rates). L1 findings establish stylised facts; they require only accurate data measurement, not identifying assumptions.

\paragraph{L2 --- Predictive.} Out-of-sample forecast performance, independent of any causal interpretation of model parameters. The principal L2 result is the Diebold-Mariano-Harvey-Leybourne-Newbold forecast comparison of Chapter~\ref{ch:results_tvp} \cite{diebold1995comparing, harvey1997testing}, with significance assessed via the Hansen-Lunde-Nason Model Confidence Set \cite{hansen2011model}.

\paragraph{L3 --- Quasi-causal.} Identification of an average treatment effect on the treated (ATT) under explicit assumptions on a non-randomised assignment mechanism. \textbf{L3.a Point identification} applies modern DiD estimators \cite{sun2021estimating, borusyak2024revisiting, callaway2021difference} and conditional-independence matching (IPWRA), covering the four carrot-policy effects of Chapter~\ref{ch:results_did} and the offtake-commitment effect of Chapter~\ref{ch:results_offtake}. \textbf{L3.b Partial identification} applies honest-DiD bounds \cite{rambachan2023more}, Oster sensitivity \cite{oster2019unobservable}, and partial-identification analysis \cite{manski2003partial}. L3 claims are asserted with their multi-estimator robustness profile and their identifying caveats, not as definitively structurally identified.

\paragraph{L4 --- Structural.} Identification of parameters of an explicit generative model that admits counterfactual computation. The Bayesian state-space carbon-conditional hazard model with GAS-driven $\bint(t)$ in Chapter~\ref{ch:results_tvp} is the principal L4 result. The five-channel real-options framework of Chapter~\ref{ch:theory} is an L4-level organising claim; three formal identification tests (Appendix~\ref{app:identification-tests}) show its constituent channels are not empirically structurally separable in the available observational sample, and the framework therefore functions as a disciplined interpretive layer rather than as a directly identified channel decomposition. The counterfactual scenarios of Chapter~\ref{ch:counterfactual} are L4 extrapolations under explicit structural-invariance assumptions and remain vulnerable to the \cite{lucas1976econometric} critique if read as forecasts.

Robustness \emph{across} levels of the hierarchy is the strongest form of evidence this dissertation offers; single-level claims are reported with calibrated language.
