...
\noindent\emph{Identification status: every counterfactual scenario reported in this chapter is an L4 extrapolation, constructed by multiplying the source-chapter treatment effect (L3.a in Chapters \ref{ch:results_did} and \ref{ch:results_offtake}) by the target subpopulation size and the three-year cumulative horizon, under explicit assumptions of structural invariance over the projection window, additivity across mechanisms, and absence of general-equilibrium feedback. The bootstrap confidence intervals quantify only the sampling uncertainty in the underlying ATE; they do not quantify the structural-invariance uncertainty. The scenarios are accordingly to be read as decision-relevant illustrations rather than as point predictions of realised outcomes.}

\noindent\emph{Central economic claim: counterfactuals remain structurally fragile; the projected impact magnitudes are conditional on structural-invariance assumptions over the projection window and on the absence of general-equilibrium feedback, and should be read as decision-relevant illustrations of the per-mechanism leverage rather than as predictions of realised outcomes.}
