...
The v7 sample, used in Chapters 5--7, does not contain end-use classification. Our Specification 2 (project-level vintage DiD) is therefore estimated only on the S\&P sample, where end-use classification is available. The cross-walk between v7 and S\&P, which would augment v7 with end-use information for the projects that overlap, is a natural future-work direction. We have begun this cross-walk using project name and structural-characteristic matching, and we describe the preliminary results in Appendix A.
